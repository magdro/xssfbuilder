...
public class MarkerManager {
	...
	Iterator<Entry<String, List<Marker>>> outer = markers.entrySet()
		.iterator();
	while (outer.hasNext()) {
	    Iterator<Marker> inner = outer.next().getValue().iterator();
	    while (inner.hasNext()) {
		Marker marker = inner.next();
		if (marker.getCol() == col && marker.getRow() == row
			&& marker.getSheetName().compareTo(sheet) == 0) {
		    inner.remove();
		}
	    }
	}
	...
	return markers.containsKey(tag) ? markers.get(tag)
